    ...
    set ROOT_DIR=D:\DATN\use
    set VERSION=7.1.1

    set PLUGIN_NAME=use-cap-%VERSION%.jar
    set USE_ZIP=use-%VERSION%.zip

    set PLUGIN_JAR=%ROOT_DIR%\use-cap\target\%PLUGIN_NAME%
    ...
    echo =====================================
    echo STEP 1 - BUILD use-cap
    echo =====================================

    cd /d %ROOT_DIR%
    call mvn clean package -pl use-cap -am
    IF %ERRORLEVEL% NEQ 0 (
        echo Build use-cap FAILED
        pause
        exit /b 1
    )
