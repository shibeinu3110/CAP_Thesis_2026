...
:
    ...
    (   generalClassifierDefinition[$n]
      ...
      | ca=capAnnotation { capAnnos.add($ca.ann); }
    )*
    { $n.setCapAnnotations(capAnnos); }
    EOF
    ;
...
capAnnotation returns [CAPAnnotation ann]
@init {
...
capArg returns [Map.Entry<String,Object> entry]
:
   key=IDENT '=' value=capValue
   {
       $entry = new java.util.AbstractMap.SimpleEntry<>($key.getText(), $value.val);
   }
;
...
:
   ...
   | n=INT         { $val = Integer.parseInt($n.getText()); }
   | f=FLOAT       { $val = Double.parseDouble($f.getText()); }
   | t='true'      { $val = Boolean.TRUE; }
   | f2='false'    { $val = Boolean.FALSE; }
   | q=qualifiedName { $val = $q.text; }

   // enum
   | e=enumRef       { $val = $e.text; }

   // nested annotation
   | ann=capAnnotation { $val = $ann.ann; }

   // annotation block: { @A(), @B() }
   | blk=capBlock { $val = $blk.list; }
...
:
    i1=IDENT { sb.append($i1.getText()); }
    ( '.' i2=IDENT { sb.append('.').append($i2.getText()); } )*
    { $text = sb.toString(); }
;
...
returns [String text]
:
    t=IDENT COLON_COLON c=IDENT
    { $text = $t.getText() + "::" + $c.getText(); }
;

capBlock returns [List<Object> list]
@init { $list = new ArrayList<Object>(); }
:
    '{'
      (
        v=capValue { $list.add($v.val); }
        (',' v=capValue { $list.add($v.val); })*
      )?
    '}'
;
